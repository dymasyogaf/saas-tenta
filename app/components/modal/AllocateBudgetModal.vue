          ...
          <div class="space-y-5">
            ...
            <div>
              ...
              <select 
                ...
                class="w-full px-4 py-2.5 bg-ink-50 border border-ink-200 rounded-xl text-ink-900 focus:outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500 text-sm font-medium"
              >
            ...
            <div>
              ...
              <div class="relative">
                <span class="absolute left-4 top-1/2 -translate-y-1/2 text-ink-500 font-bold text-base">{{ isGlobal ? '$' : 'Rp' }}</span>
                <input 
                  type="text" 
                  v-model="allocateAmountInput" 
                  @input="formatAllocateInput" 
                  placeholder="1.000.000" 
                  class="w-full pl-11 pr-4 py-3 bg-white border border-ink-200 rounded-xl text-ink-900 text-lg font-bold focus:outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20 transition-all placeholder:font-normal placeholder:text-ink-300 shadow-2xs" 
                />
              </div>
              <p v-if="allocateAmount > availableBalance" class="text-xs text-red-500 font-medium mt-1.5 flex items-center gap-1">
                <Info class="w-3.5 h-3.5 shrink-0" /> Saldo Tersedia tidak mencukupi
              </p>
            </div>
          </div>
