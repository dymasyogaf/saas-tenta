    ...
    <div class="fixed inset-0 flex items-center justify-center p-4">
      <div class="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden relative transform transition-all">
        ...
        <div class="flex items-center justify-between p-6">
          <h3 class="font-bold text-ink-900 text-xl">Ubah Email</h3>
        ...
        <div class="px-6 space-y-5">
          <p class="text-sm text-ink-500 mb-2">
            Email saat ini: <span class="font-semibold text-ink-900">{{ currentEmail }}</span>
          </p>
          
          <!-- Email Baru -->
          <div>
            <label class="block text-sm font-medium text-ink-900 mb-2">Email Baru</label>
            <div class="relative">
              <input 
                v-model="newEmail" 
                type="text" 
...
const saveEmail = async () => {
  ...
  try {
    ...
    addToast(err.message || 'Gagal mengubah email', 'error')
  } finally {
