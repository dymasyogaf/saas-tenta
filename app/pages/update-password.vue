  ...
  <div class="min-h-screen flex items-center justify-center p-4 font-sans antialiased bg-[url('data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iMjAiIGhlaWdodD0iMjAiIHhtbG5zPSJodHRwOi8vd3d3LnczLm9yZy8yMDAwL3N2ZyI+PGNpcmNsZSBjeD0iMSIgY3k9IjEiIHI9IjEiIGZpbGw9InJnYmEoMTQsIDI3LCA0MiwgMC4wMykiLz48L3N2Zz4=')]">
    <div class="w-full max-w-md bg-white rounded-3xl shadow-xl shadow-ink-900/5 border border-ink-100 p-8 md:p-10 relative overflow-hidden">
      ...
      <div class="text-center mb-8">
        <h1 class="text-2xl font-display font-bold text-ink-900 mb-2">{{ $t('auth.resetPasswordTitle') }}</h1>
        <p class="text-ink-500 text-sm">{{ $t('auth.resetPasswordSubtitle') }}</p>
      </div>

      <form @submit.prevent="handleUpdatePassword" class="space-y-5">
        <div>
          <label class="block text-sm font-semibold text-ink-900 mb-2">{{ $t('auth.newPassword') }}</label>
          <div class="relative">
            <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-ink-400">
              <Lock class="w-5 h-5" />
            </div>
            <input v-model="newPassword" :type="showNew ? 'text' : 'password'" required :placeholder="$t('auth.min8Chars')" class="w-full pl-11 pr-12 py-3 bg-ink-50 border border-ink-200 rounded-xl focus:bg-white focus:border-orange-500 focus:ring-4 focus:ring-orange-500/10 transition-all outline-none text-ink-900 placeholder:text-ink-400">
        ...
        <div>
          <label class="block text-sm font-semibold text-ink-900 mb-2">Konfirmasi {{ $t('auth.newPassword') }}</label>
          <div class="relative">
            <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-ink-400">
              <Lock class="w-5 h-5" />
            </div>
            <input v-model="confirmPassword" :type="showConfirm ? 'text' : 'password'" required :placeholder="$t('auth.repeatNewPassword')" class="w-full pl-11 pr-12 py-3 bg-ink-50 border border-ink-200 rounded-xl focus:bg-white focus:border-orange-500 focus:ring-4 focus:ring-orange-500/10 transition-all outline-none text-ink-900 placeholder:text-ink-400">
      ...
      <p class="text-center text-sm mt-6">
        <NuxtLink to="/login" class="text-ink-500 hover:text-orange-600 font-medium transition-colors">{{ $t('auth.backToLogin') }}</NuxtLink>
      </p>
    </div>
  </div>
</template>
...
const handleUpdatePassword = async () => {
  if (newPassword.value !== confirmPassword.value) {
    addToast(t('auth.toast.passwordMismatch'), 'error')
    return
  }
  ...
  try {
    ...
    addToast(t('auth.toast.updateSuccess'), 'success')
    router.push('/login')
  } catch (err: any) {
    addToast(err.message || t('auth.toast.updateFailed'), 'error')
  } finally {
    loading.value = false
  }
}
